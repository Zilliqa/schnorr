...
BOOST_AUTO_TEST_CASE(test_curve_setup) {
  ...
  unique_ptr<char, void (*)(void*)> order_actual(
      BN_bn2hex(Schnorr::GetCurveOrder()), free);
  BOOST_CHECK_MESSAGE(strcmp(order_expected, order_actual.get()) == 0,
                      "Wrong order generated");

  unique_ptr<char, void (*)(void*)> basept_actual(
      EC_POINT_point2hex(Schnorr::GetCurveGroup(),
                         EC_GROUP_get0_generator(Schnorr::GetCurveGroup()),
  ...
  if ((a != nullptr) && (b != nullptr) && (p != nullptr) && (h != nullptr)) {
    BOOST_CHECK_MESSAGE(
        EC_GROUP_get_curve_GFp(Schnorr::GetCurveGroup(), p.get(), a.get(),
                               b.get(), NULL) != 0,
        "EC_GROUP_get_curve_GFp failed");
    BOOST_CHECK_MESSAGE(
        EC_GROUP_get_cofactor(Schnorr::GetCurveGroup(), h.get(), NULL) != 0,
...
BOOST_AUTO_TEST_CASE(test_keys) {
  ...
  BOOST_CHECK_MESSAGE(
      BN_cmp(keypair.first.m_d.get(), Schnorr::GetCurveOrder()) == -1,
      "Key generation check #1 failed");
  BOOST_CHECK_MESSAGE(BN_is_zero(keypair.first.m_d.get()) != 1,
                      "Key generation check #2 failed");

  BOOST_CHECK_MESSAGE(
      EC_POINT_mul(Schnorr::GetCurveGroup(), P.get(), keypair.first.m_d.get(),
                   NULL, NULL, NULL) != 0,
      "Key generation check #3 failed");
  BOOST_CHECK_MESSAGE(
      EC_POINT_cmp(Schnorr::GetCurveGroup(), keypair.second.m_P.get(), P.get(),
                   NULL) == 0,
...
BOOST_AUTO_TEST_CASE(test_sign_verif) {
  ...
  std::vector<uint8_t> message_rand(message_size);
  std::vector<uint8_t> message_1(message_size, 0x01);
...
BOOST_AUTO_TEST_CASE(test_performance) {
  ...
  for (unsigned int i = 0; i < num_messages; i++) {
    std::vector<uint8_t> message_rand(message_sizes[i]);
    generate(message_rand.begin(), message_rand.end(), std::rand);
    ...
    BOOST_CHECK_MESSAGE(Schnorr::Sign(message_rand, keypair.first,
                        ...
                        "Signing failed");
    cout << "Message size  = " << printable_sizes[i] << endl;
    cout << "Sign (usec)   = " << r_timer_end(t) << endl;
    ...
    BOOST_CHECK_MESSAGE(
        ...
        "Signature verification (correct message) failed");
    cout << "Message size  = " << printable_sizes[i] << endl;
    cout << "Verify (usec) = " << r_timer_end(t) << endl;
  }
}
...
BOOST_AUTO_TEST_CASE(test_serialization) {
  ...
  std::vector<uint8_t> privkey_bytes, pubkey_bytes, signature_bytes;
...
BOOST_AUTO_TEST_CASE(test_error_deserialization_pubkey) {
  PubKey pubkey;
  std::vector<uint8_t> pubkey_bytes_empty;
  bool returnValue = pubkey.Deserialize(pubkey_bytes_empty, 0);
  BOOST_CHECK_MESSAGE(returnValue == false,
                      "Expected: false Obtained: " << returnValue);
}
...
BOOST_AUTO_TEST_CASE(test_error_deserialization_privkey) {
  PrivKey privkey;
  std::vector<uint8_t> privkey_bytes_empty;
  bool returnValue = privkey.Deserialize(privkey_bytes_empty, 0);
  BOOST_CHECK_MESSAGE(returnValue == false,
                      "Expected: false Obtained: " << returnValue);
}
...
BOOST_AUTO_TEST_CASE(test_error_deserialization_signature) {
  Signature signature;
  std::vector<uint8_t> sig_bytes_empty;
  bool returnValue = signature.Deserialize(sig_bytes_empty, 0);
  BOOST_CHECK_MESSAGE(returnValue == false,
                      "Expected: false Obtained: " << returnValue);
}
